...
const MenuItem = ({ icon, text, isSelected }) => {
  return (
    ...
    <div
      ...
      className={`flex items-center py-2 px-7 group cursor-pointer ${
        isSelected ? "bg--200 rounded-md" : ""
      }`}
    >
...
const StatCard = ({ title, value, icon,bgColor, textColor, imageSrc }) => {
  return (
    <div className={`p-6 rounded-lg shadow-md ${bgColor} ${textColor} flex items-center justify-between`}>
      ...
      {imageSrc ? (
        // Using a more generic placeholder URL that renders a simple icon/text if you don't have an exact image.
        // For the specific image, you would need to add it to your public folder or a CDN.
...
function Dashboard() {
  ...
  return (
    <div className="flex h-screen bg-white"> {/* Overall container, if sidebar has rounded corners, this needs to be bg-white */}
      ...
      <div className="realtive w-64 h-[750px] flex flex-col justify-between py-[94px] px-4 shadow-lg "
    
      style={{
         backgroundImage: `url('/side.svg')`,  // Make sure this path is correct
      backgroundRepeat: 'no-repeat',
      backgroundSize: 'cover', // or 'contain' depending on your design
      backgroundPosition: 'right', // or 'center' or 'top' as per design
      }}> 
        <div>
          ...
          <nav className="space-y-1">
           <MenuItem icon="/menu.svg" text="Menu" />
            <div
      className="rounded-full bg-white shadow-md p-4  mb-6"
      style={{
        width: '240px',    // Width of the rounded box
        height: '55px',
    ...
    >
      <div className="flex items-center space-x-3">
        ...
        <span className="text-xl font-medium">Dashboard</span>
            ...
            {/* <MenuItem icon="/des.svg" text="Dashboard" isSelected={true} />  */}
            <MenuItem icon="/Frame.svg" text="Analytics" />
            <MenuItem icon="/frame1.svg" text="Staff details" />
            <MenuItem icon="/pay.svg" text="Payment" />
            <MenuItem icon="/frame2.svg" text="Inventory" />
            <MenuItem icon="/feed.svg" text="Feedback" />
           <MenuItem icon="/book.svg" text="Menu Section" />
           <MenuItem icon="/frame3.svg" text="Booking" />
           <MenuItem icon="/Vector1.svg" text="Setting" />
      ...
      {/* Main Content Area - Overall background white, padding added to the container */}
...
<div className="flex items-stretch justify-between bg-white rounded-xl shadow-md p-6  h-[200px] w-[2000px] max-w-none mx-auto bg-gradient-to-br from-[#FFEAD2] to-[#FFFFFF] lg:h-[250px] lg:w-full border border-gray-300 mb-6 mt-[-10px]">
  ...
  <div key={data.stars} className="flex items-center mb-2">
    {/* Left side text */}
    <span className="w-20  text-gray-700">{data.stars} star</span>
    ...
    {/* Right side count */}
    <span className="w-16 text-right text-gray-700">{data.count}</span>
  </div>
))}
